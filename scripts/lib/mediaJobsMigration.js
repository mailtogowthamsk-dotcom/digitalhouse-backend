/**
 * Idempotent media_jobs + media_files processingStatus migration.
 * Shared by legacy npm script and versioned db/migrations runner.
 */
 ...
 */
async function runMediaJobsMigration(conn) {
  ...
  if (await columnExists(conn, "media_files", "processingStatus")) {
    ...
    if (alreadyCurrent) {
      if (statusValues.includes("pending_upload") || statusValues.includes("ready")) {
        await conn.query(`
          UPDATE media_files
  ...
  await addIndexIfMissing(conn, "media_jobs", "uq_media_jobs_media", ["mediaId"], true);
  await addIndexIfMissing(conn, "media_jobs", "idx_media_jobs_claim", [
    "status",
    "updatedAt",
    "createdAt"
  ]);
  await addIndexIfMissing(conn, "media_jobs", "idx_media_jobs_worker", [
    "workerId",
    "status"
  ]);

  if (!(await foreignKeyExists(conn, "media_jobs", "mediaId", "media_files", "id"))) {
...
module.exports = { runMediaJobsMigration };
